...
parser = argparse.ArgumentParser(description='Simple Contrastive Loss MNIST/EMNIST Pretrained MNIST')
...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, sampler=train_sampler,
                                           drop_last=True)
val_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, sampler=valid_sampler, drop_last=True)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    torch.save(model, 'last_alex.pth')

    model = torch.load('last_alex.pth')
    model = model.to(DEVICE)
    model.eval()

...
for epoch in range(1, args.num_epochs + 1):
    ...
    torch.save(model, 'emnist_n.pth')

    model = torch.load('emnist_n.pth')
    model = model.to(DEVICE)
    model.eval()
    ...
    for dataloader in [train_loader, test_loader]:
        ...
        for batch in dataloader:
            ...
            for index, label in enumerate(label1.numpy()):
                if label in embbedding_dict.keys():
                    existing_emb_for_label = embbedding_dict.get(label)
                    existing_emb_for_label.append(out1[index].data[:])
                else:
                    embbedding_dict[label] = [out1[index].data[:]]
            for index, label in enumerate(label2.numpy()):
                if label in embbedding_dict.keys():
                    existing_emb_for_label = embbedding_dict.get(label)
                    existing_emb_for_label.append(out2[index].data[:])
                else:
                    embbedding_dict[label] = [out2[index].data[:]]
